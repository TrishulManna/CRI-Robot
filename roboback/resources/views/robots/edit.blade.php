...
@section('content')
    ...
    <div class="container">
        ...
        <div class="form-group">
         ...
         <div class="form-group">
            @if((bool)App\RoleUsers::where('user_id', Illuminate\Support\Facades\Auth::id())->where('role_id', 1)->first() || (bool)App\RoleUsers::where('user_id', Illuminate\Support\Facades\Auth::id())->where('role_id', 2)->first())
              {!! Form::label('text', 'Gebruiker:', ['class' => 'control-label']) !!}
              {!! Form::select('user_id', App\User::getUsersForSelect() , ['class' => 'form-control']) !!}
            @else
              {!! Form::label('text', 'Gebruiker:', ['class' => 'control-label']) !!}
              {!! Form::select('user_id', array(Illuminate\Support\Facades\Auth::id() => Illuminate\Support\Facades\Auth::user()->name) , ['class' => 'form-control', 'disabled', 'readonly']) !!}
            @endif
        </div>       
